...
export class ItemizedReceiptForm {
  ...
  addItemRow(focus = true, insertIndex = -1): void {
    const today = new Date().toLocaleDateString('en-US');
    const newId = 'item-' + Date.now();
    const newItem: ItemizedReceiptItem = {
      id: newId,
      dateOfService: today,
      ordNumber: '0',
      sku: '',
    ...
    if (focus) {
      setTimeout(() => {
        ...
        if (targetRow) {
          const descInput = targetRow.querySelector('input[data-item-field="description"]') as HTMLInputElement;
  ...
  addDiscountRow(parentIndex = -1, focus = true): void {
    const today = new Date().toLocaleDateString('en-US');
    ...
    const newDiscItem: ItemizedReceiptItem = {
      id: newId,
      dateOfService: dos,
      ordNumber: '',
      sku: '',
    ...
    if (focus) {
      setTimeout(() => {
        ...
        if (row) {
          const amtInput = row.querySelector('input[data-item-field="amount"]') as HTMLInputElement;
          if (amtInput) {
            amtInput.focus();
            amtInput.select();
          }
