...
export class ItemizedReceiptForm {
                ...
                if (field === 'patientName') {
                    ...
                    if (couponChart && couponChart !== target) {
                        if ('value' in couponChart) {
                            couponChart.value = target.value;
                        }
                        else {
                            couponChart.textContent = target.value;
                        }
                    }
                    const metaChart = this.container.querySelector('.ir-input-meta[data-field="chartNumber"]');
    ...
    calculateTotals(triggerSave = true) {
        ...
        if (this.state.autoCalculateTax) {
            ...
            const taxInput = this.container.querySelector('#ir-input-tax');
        ...
        if (dispSumTotalCharges)
            dispSumTotalCharges.textContent = chargesWithTax.toFixed(2);
